...
interface MemorablesRequestBody {
  material?: unknown;
  summary?: unknown;
...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as MemorablesRequestBody;
    ...
    const data = await runMemorableChain({
      material,
      summary,
      recentMessages,
      rollingSum,
    ...
    const response: ApiResponse<{ memorables: string[] }> = {
      success: false,
